...
public class LobbyManager : GenericNetworkCore
{
    ...
    public IEnumerator SlowAgentStart()
    {
        ...
        if(!UsePublic)
        {
            GenericNetworkCore.Logger("Trying Florida Poly Address: " + IntranetIP.ToString());
            pr = ping.Send(IntranetIP, timeout, buffer, po);
            yield return new WaitForSeconds(1.5f);
            Debug.Log("Ping Return: " + pr.Status.ToString());
    ...
    public void UpdatePlayers(string g)
    {
        if (g.StartsWith("PLAYERS#"))
        {
            ...
            if (IsMaster)
            {
                foreach (KeyValuePair<int, Connector> con in Connections)
                {
                    Send(g, con.Key);
                }
            }
  ...
  /// <param name="g"></param>
    ...
    public IEnumerator RoomKiller(float t, GameRoom g )
    {
        if (IsMaster)
        {
            ...
            foreach(KeyValuePair<int,Connector> c in Connections)
            {
